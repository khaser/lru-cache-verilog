...
\newminted{sv}{ fontsize=\scriptsize, bgcolor=bg, breakafter=, breaklines, breakautoindent=true, breaksymbolleft=\raisebox{0.8ex}{ \small\reflectbox{\carriagereturn}}, breaksymbolright=\small\carriagereturn, }
...
Прежде всего, необходимо уточнить, как в моём коде
реализованы inout шины на Verilog. Для примера
...
Вторым пунктом, я хочу отметить, что размер шин адреса 
\var{addr1_bus_size} и \var{addr2_bus_size} у меня округлены
с 15 бит до 16, для того, чтобы все шины единообразно
задавались целым количеством байт, мне кажется это немного
упростило внутренний код модулей. 

Давайте на примере команды \var{C2_WRITE_LINE}
отправленной от кэша памяти
рассмотрим как происходит синхронизация между двумя схемами.
Основная идея по синхронизации схем: память отрабатывает
во время, когда такт синхронизации 0, то есть после 
\mintinline{sv}{negedge clk}, а кэш~--- наоборот
во время синхронизации 1, то есть после
\mintinline{sv}{posedge clk}. Эти меры позволяют схеме
работать максимально предсказуемо и писать код с меньшим
количеством ошибок. Но это имеет и свои минусы.
Самый главный из них это не очевидная оценка работы из-за
следующего эффекта. Допустим кэш хочет начать считывание
из оперативной памяти. Первым делом, он ждёт события
\mintinline{sv}{posedge clk} и вот тут начинается проблема.
В зависимости от состояния синхронизации один такт может 
пропасть просто на ничего не деланье. Поэтому оценка времени
работы либо должна это учитывать, что не так просто, так как
надо по-честному знать время исполнение каждой 
команды, либо смириться с тем, что у нас есть ошибка
в среднем в половину такта при каждом обращении к памяти.
Вторая проблема, возникающая из-за строгих привязок к 
posedge\slash backedge это выставление внутренних задержек
внутри модуля. А именно, если внутри памяти выставить задержку
в один такт, то она не окажет никакого эффекта, так как 
этот такт всё равно она прождёт, во время синхронизации. 

\textbf{И тут нам подвинули дедлайн и я решил всё таки сделать clk нормально.}
А именно, если раньше за такт считалось время, на 
протяжении которого clk удерживал одно и то же значение, то теперь
за один такт взято расстояние от одного \mintinline{sv}{posedge clk} до следующего.
это полностью решает описанные выше проблемы с чётностью\slash нечётностью
такта. В истории git можно посмотреть внесённые для этого
перехода изменения. 
...
Заметим, что время работы достаточно отличается от
прогнозированного чуть меньше, чем на 20\%, 
я могу выделить несколько основных причин, почему это произошло. 

\begin{itemize}
    \item 
        Код на C++ писался до кода на Verilog и количество
        тактов, которое будет уходить на передачу тех или
        иных данных было спрогнозировано достаточно недостаточно точно.
        Например изначально не было учтены такты необходимые
        на передачу владения шиной от одного модуля к другому. 
        Можно думать, что при каждом обращении к кэшу есть
        ошибка в среднем в один такт.
    \item
        В коде на Verilog много где сформирована модель, что
        принимающая сторона синхронизируется по \mintinline{sv}{posedge clk},
        а отдающая по \mintinline{sv}{negedge clk} или наоборот.
        Из-за этого очередная операция считывания может быть начата
